...
# config
my $HEADER_PATTERNS = "video-pixels-patterns.h";
my $HEADER_GEGL_PROP = "video-pixels-gegl-prop.h";
my $PATTERN_DIR = "patterns";
my $GEGL_PROP_PREFIX = "GEGL_VIDEO_DEGRADATION_TYPE";
my $GEGL_PROP_TEMPLATE = '
...
property_enum (pattern, _("Pattern"), GeglVideoDegradationPlusType,
    gegl_video_degradation_type, __DEFAULT_ENUM_VALUE__)
...
sub main {
    ...
    open(my $gfh, ">", $HEADER_GEGL_PROP) or error("Cannot open '$HEADER_GEGL_PROP' for writing");
    write_preamble($pfh);
    write_preamble($gfh);
    ...
    foreach my $filename (@files) {
        ...
        $gegl_enum_text .= generate_gegl_enum_text($pattern);
    }
    write_patterns_array($pfh, $patterns);
    write_gegl_prop_file($gfh, $patterns, $gegl_enum_text);
...
sub write_preamble {
    ...
    print($fh "/*\n");
    print($fh " * Do not edit this file.  It is automatically generated by generate-patterns.pl\n");
    print($fh " * based on xpm image files in the \"patterns\" subdirectory.  Any changes here\n");
    print($fh " * may be overwritten.\n");
    print($fh "*/\n\n");
}
...
sub write_gegl_prop_file {
    my ($fh, $patterns, $gegl_enum_text) = @_;
    my $default_prop = get_gegl_property($patterns->[0]);
    $gegl_enum_text =~ s/\s+$//;
    my $template = $GEGL_PROP_TEMPLATE;
    $template =~ s/^\s+//;
...
sub get_gegl_property {
    my ($pattern) = @_;
    return "${GEGL_PROP_PREFIX}_" . uc(get_c_identifier($pattern));
}
