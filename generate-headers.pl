...
sub main {
    ...
    write_gegl_enum_file($gfh, $patterns, $gegl_enum_text);
...
sub write_preamble {
    ...
    print($fh "/*\n");
    print($fh " * Do not edit this file.  It is automatically generated by generate-headers.pl\n");
...
sub write_gegl_enum_file {
    ...
    $gegl_enum_text =~ s/\s+$//;
    my $template = $GEGL_ENUM_TEMPLATE;
    $template =~ s/^\s+//;
...
sub get_gegl_property {
    my ($pattern) = @_;
    return "${GEGL_ENUM_PREFIX}_" . uc(get_c_identifier($pattern));
}
